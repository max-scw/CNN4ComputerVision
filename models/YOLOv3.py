...
from BoundingBoxRegression import draw_box
...
if __name__ == "__main__":
    model = load_yolov3_model(pl.Path("../trained_models") / "2210310931_YOLOv3tiny-416x416-6x2_rgb")

    model = YOLOv3(input_shape=(1024, 1024),
                   num_classes=2,
    ...
    model.load_model(pl.Path("../trained_models") / "2210301424_YOLOv3_rgb.h5")  # tiny YOLO
...
class YOLOv3:
    ...
    def yolo_loss(
        ...
        input_shape = K.cast(K.shape(yolo_outputs[0])[1:3] * 32, K.dtype(y_true[0]))  # FIXME: height, width??
        grid_shapes = [K.cast(K.shape(yolo_outputs[i])[1:3], K.dtype(y_true[0])) for i in range(self.num_scale_levels)]
        loss = 0
        m = K.shape(yolo_outputs[0])[0]  # batch size, tensor
        mf = K.cast(m, K.dtype(yolo_outputs[0]))
        ...
        for i_anc, anc in enumerate(best_anchor):
            ...
            for i_lyr in range(num_layers):
                ...
                if anc in anchor_mask[i_lyr]:
                    ...
                    # copy true box coordinates to the output matrix/vector
                    # y_true: list(numpy.ndarray) => [(batch_size, grid_shape, num_anchors, num_classes + 5]
                    y_true[i_lyr][i_bsz, j, i, idx_anc, 0:4] = true_boxes[i_bsz, i_anc, 0:4]  # FIMXE: error
                    y_true[i_lyr][i_bsz, j, i, idx_anc, 4] = 1
                    y_true[i_lyr][i_bsz, j, i, idx_anc, 5 + class_id] = 1
    return y_true
